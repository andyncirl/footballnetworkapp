...
class StaticPagesController < ApplicationController
  ...
  def news
    begin
      ...
      @international_football_posts = RSS::Parser.parse(open('http://www.skysports.com/rss/0,20514,12010,00.xml').read, false).items[0...10]

    rescue
      # Do nothing, just continue.  The view will skip the football post section if the feed is nil.
      @latest_football_posts = nil
      @international_football_posts = nil
    end
    end
  end